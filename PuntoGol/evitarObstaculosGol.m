...
function [angR,vD]=evitarObstaculosGol(ranges,sensorx_R,sensory_R,sensorAngle_R,x,y,theta,xd,yd)
rangesF=ranges(1,:);
...
%    dir_o=dir+Pv;
   O_G=(Rthetav*O_R)+Pv;
   dir_o=O_G-Pv;
   dir_eo=atan2(dir_o(2),dir_o(1))+pi;
   ang=abs(atan2(dir(2),dir(1)));
   angR=theta-dir_eo;
   vD=0.1;
%    if(((theta>19*pi/40))&&(ang>9*pi/20))
%    if(((theta>19*pi/40))&&((ang>9*pi/20)||(33*pi/20>ang)))
%        angR=wrapToPi(theta-dir_eo);
%    else
%        angR=0
%    end
%    vD=0.1;
